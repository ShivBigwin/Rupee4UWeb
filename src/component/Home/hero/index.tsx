...
export default function Hero() {
  ...
  const slides = [
    ...
    },
    {
      ...
      subtitle: "Quick approval process with competitive rates to fuel your personal growth and expansion",
  ...
  const particles = Array.from({ length: 40 }, (_, i) => ({
    ...
    delay: Math.random() * 2,
    color: i % 3 === 0 ? 'bg-blue-200' : i % 3 === 1 ? 'bg-cyan-200' : 'bg-emerald-200'
  }));
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-linear-to-br from-gray-50 via-white to-blue-50">
      ...
      <div className="relative z-10 container mx-auto px-4 py-20 lg:py-32">
          ...
          <div className="relative h-full flex items-center px-4 md:px-12 lg:px-24 ">
            ...
            >
              ...
              >
                ...
                <span className="text-blue-700 text-sm font-medium">Smart Finance</span>
              </motion.div>
              ...
              >
                <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  ...
                  >
                    {/* Button Background */}
                    <span className={`absolute inset-0 bg-gradient-to-r ${slides[currentSlide].gradient} opacity-95 group-hover:opacity-100 transition-opacity duration-300 shadow-lg`} />
          ...
          <div className="absolute bottom-6 md:bottom-8 left-1/2 transform -translate-x-1/2 z-20 flex space-x-3">
            {slides.map((_, index) => (
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
              >
                <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full transition-all duration-300 ${index === currentSlide
                  ? `bg-gradient-to-r ${slides[index].gradient}`
                  : 'bg-gray-400/50 group-hover:bg-gray-600'
                  }`} />
                <div className={`absolute -inset-1 md:-inset-2 rounded-full border-2 ${index === currentSlide
                  ? 'border-white/50'
                  : 'border-transparent'
          ...
          <div className="absolute inset-y-0 left-2 md:left-4 right-2 md:right-4 flex items-center justify-between pointer-events-none">
            ...
            >
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-full bg-transparent backdrop-blur-sm border border-gray-200 flex items-center justify-center group-hover:border-blue-500/50 transition-colors duration-300 shadow-lg">
                <svg className="w-5 h-5 md:w-6 md:h-6 text-white group-hover:text-blue-600 transition-colors duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        >
          {[
            { value: "Instant", label: "Approval Time", desc: "Fast Processing", gradient: "from-blue-800 to-cyan-500" },
            { value: "₹50Cr+", label: "Funds Disbursed", desc: "Trusted by 5000+ Businesses", gradient: "from-blue-800 to-cyan-500" },
            { value: "9.8/10", label: "Customer Rating", desc: "Excellent Service", gradient: "from-blue-800 to-cyan-500" }
          ].map((stat, index) => (
            <motion.div
              key={index}
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className={`text-2xl md:text-3xl font-bold bg-linear-to-r ${stat.gradient} bg-clip-text text-transparent`}>
                  {stat.value}
                </span>
                <motion.div
              ...
              <h3 className="text-gray-800 text-sm md:text-base font-semibold mb-1">{stat.label}</h3>
        ...
        >
          {["RBI Approved", "ISO Certified", "24/7 Support"].map((badge, index) => (
            <motion.div
              key={index}
              whileHover={{ y: -3 }}
